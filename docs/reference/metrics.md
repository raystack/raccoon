...
* Type: `Count`
* Tags: `reason=ugfailure` `reason=exists` `reason=serverlimit` `conn_type=*`
...
* Type: `Count`
* Tags: `success=false` `success=true` `conn_type=*`
...
* Type: `Count`
* Tags: `status=failed` `status=success` `reason=*` `conn_type=*`
