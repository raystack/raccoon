...
describe('retry', () => {
    test('should return callback result on first success attempt', async () => {
        ...
        const result = await retry(mockCallback, 3, 100);
    ...
    test('should throw error if callback always fails', async () => {
        ...
        await expect(retry(mockCallback, 3, 100)).rejects.toThrow('All attempts failed');
        expect(mockCallback).toHaveBeenCalledTimes(3);
    });
    ...
    test('should throw error if retry limit exceeded', async () => {
        ...
        await expect(retry(mockCallback, 3, 100)).rejects.toThrow('Attempt 3 failed');
        expect(mockCallback).toHaveBeenCalledTimes(3);
    });
    ...
    test('should apply waitTime', async () => {
        ...
        try {
            await retry(mockCallback, 3, 500);
        } catch (error) {
